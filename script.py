...
def fuzzy_group_headings(lines):
    ...
    while i < n:
        ...
        while j < n:
            ...
            # 🎯 Increased threshold for more conservative merging
            if sim >= 5:  # Changed from 4 to 5 for stricter merging
                run.append(nxt)
                used.add(j)
                curr = nxt
                j += 1
